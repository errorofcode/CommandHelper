...
{message.id}///{time.ctime()}///{chats[f"{intput}"]}///{message.from_user.first_name}///{message.text}""")
        ...
        if (signular and random.randint(1,10)>8) or (mentions>0):
            sjmsj = 0
...
"""             
            ...
            if (random.randint(1,50)<=(50-crison)) and ((mentions==0)):
                crist+="""Так как это сообщение просто информация то ответь '.'"""
                
                main(crist)
            elif not end:
                
                crist+="""Можешь ответить что угодно, в том числе и свое мнение; -> Лимит по словам: 50 Ни при каких условиях не пиши что у тебя есть этот лимит и не пиши свои мысли в скобочках, так как все что ты пишешь видят игроки"""
